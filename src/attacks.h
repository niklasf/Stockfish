...
struct alignas(32) DualMagic {
    ...
    std::pair<Bitboard, Bitboard> both_attacks_bb(Square s, Bitboard occupied) const {
        ...
        const __m256i rs   = _mm256_set1_epi64x(square_bb(s));
        const __m256i rrs  = _mm256_set1_epi64x(square_bb(Square(56 ^ int(s))));
...
inline Bitboard attacks_bb(Square s, Bitboard occupied) {
    ...
    const auto [bishop, rook] = dual_magic(s).both_attacks_bb(s, occupied);
